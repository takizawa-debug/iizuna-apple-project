...
(function () {
    ...
    const type = params.get('type') || 'shop'; // shop, event, farmer, other
    const lang = params.get('lang') || 'ja';

    const strings = i18n; // Assuming i18n handles current language based on URL internally via getLang() in i18n.js

    const typeConfig = strings.types[type];
    if (!typeConfig) {
        document.getElementById('print-root').innerHTML = "Invalid form type.";
        return;
    }
    ...
    const box = (type = 'text', subtext = '') => `
    <div class="field-input">
      <div class="input-box ${type === 'textarea' ? 'textarea' : type === 'textarea-large' ? 'textarea-large' : ''}"></div>
    ...
    html += row(label(typeConfig.cat_label, true) + checkboxList([
    ...
    // 描画後、少し待ってから自動で印刷ダイアログを起動(一応ユーザーアクションで出せるようにボタンも配置済み)
