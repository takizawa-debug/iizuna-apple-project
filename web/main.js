...
// あらゆる画像のURLを最適化する「賢い変換くん」
const fixUrl = (url) => {
  if (!url) return "";
  
  let newUrl = url;

  // A. ペライチS3があればCDNに変換（高速化）
  newUrl = newUrl.replace(
    "https://s3-ap-northeast-1.amazonaws.com/s3.peraichi.com/",
    "https://cdn.peraichi.com/"
  );

  // B. Googleドライブの共有リンクがあれば直リンクに変換（表示不可対策）
  if (newUrl.includes("drive.google.com")) {
    const match = newUrl.match(/\/d\/([^/]+)/) || newUrl.match(/id=([^&]+)/);
    if (match) newUrl = `https://drive.google.com/uc?export=download&id=${match[1]}`;
  }

  // C. それ以外（外部URLなど）はそのまま返す
  return newUrl;
};
...
function showModalFromCard(card){
  ...
  const imageBlock = gallery.length ? `<div class="lz-mm"><img id="lz-mainimg" crossorigin="anonymous" referrerpolicy="no-referrer-when-downgrade" src="${esc(fixUrl(gallery[0]))}" alt="${esc(t)}"></div>` : '';
  const galleryBlock = (gallery.length>1) ? `<div class="lz-g" id="lz-gallery">${gallery.map((u,i)=>`<img crossorigin="anonymous" referrerpolicy="no-referrer-when-downgrade" src="${esc(fixUrl(u))}" data-img-idx="${i}" class="${i===0?'is-active':''}" alt="">`).join("")}</div>` : '';
