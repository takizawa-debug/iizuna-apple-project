...
function showModalFromCard(card){
  ...
  document.title = `${t} | ${originalTitle}`; // 【SEO強化】
  ...
  /* ====== PDF生成（S3逆変換 & エラー自動除外ロジック） ====== */
  if(window.innerWidth >= 769 && MODAL.querySelector(".lz-pdf")){
    MODAL.querySelector(".lz-pdf").onclick = async ()=>{
      ...
      try{
        ...
        imgs.forEach(img => {
          let src = img.src;
          // ペライチCDN URLを AWS S3 直URLに置換（突破口）
          ...
          // キャッシュの影響を排除するためにタイムスタンプを付与
          img.src = src + (src.indexOf('?') === -1 ? '?' : '&') + "pdf_v=" + Date.now();
          
          // 【超重要】画像がCORSエラーなどで読み込めない場合、その画像だけ非表示にしてPDF生成を続行する
          img.onerror = () => { img.style.display = 'none'; };
        });

        document.body.appendChild(clone);

        // クローン内のすべての画像が「読み込み完了」または「エラー確定」になるまで待つ
        ...
        new QRCode(qrDiv,{ text:url, width:64, height:64, correctLevel: QRCode.CorrectLevel.L });
        const qrCanvas = qrDiv.querySelector("canvas"), qrData = qrCanvas ? qrCanvas.toDataURL("image/png") : "";

        // html2canvas 実行（CORSを許可し、汚染されたキャンバスを拒否する設定）
        ...
        const imgH_mm = pt2mm(PDF_FOOTER.jpPt), imgW_mm = imgH_mm / jpImg.ar, xJP = margin, yJP = yBaseDT - imgH_mm + PDF_FOOTER.jpBaselineGapMm;
        ...
        while(heightLeft > 0){
          ...
          pdf.addImage(jpImg.data, "PNG", xJP, yJP, imgW_mm, imgH_mm);
        ...
        console.error(e); alert("PDF生成失敗。画像読み込みの制限が原因です。");
      }
    };
  }
}
