/**
 * modal.js - 詳細表示・機能コンポーネント (グローバル検索 & 優先順位色分け統合 Edition)
 * 役割: 既存機能を100%維持。window.LZ_DATA(全データ)から多言語検索。赤(直通)を最優先、緑(検索)を次点として表示。
 */
window.lzModal = (function() {
  ...
  /* 検索機能: 全データ(window.LZ_DATA)を対象に、現在の言語に合わせて精密に検索。 */
  function renderSearchResults(keyword, targetLang) {
    ...
    allData.forEach(function(item) {
      ...
      if (title.includes(keyword) || lead.includes(keyword) || body.includes(keyword) || tags.includes(keyword) || l1.includes(keyword) || l2.includes(keyword)) {
        seenIds.add(item.id);
