 ...
 * 役割: モーダル構築、URL同期、前後ナビ、SNSシェア、精密PDF生成
 */
window.lzModal = (function() {
  ...
  // 元の main.js から正確なアイコン定義と stroke-width を復元
  var ICON = {
  ...
  var injectStyles = function() {
    ...
    style.textContent = [
      ...
      '.lz-modal { position: relative; width: min(860px, 92vw); max-height: 88vh; overflow-y: auto; background: #fff; border: 2px solid var(--apple-red); border-radius: 12px; box-shadow: 0 20px 50px rgba(0,0,0,0.2); }',
      ...
      '.lz-mt { margin: 0; font-weight: 600; font-size: 2.0rem; color: var(--apple-red-strong); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1; }',
      '.lz-actions { display: flex; gap: 8px; margin-left: 10px; }',
      '.lz-btn { display: inline-flex; align-items: center; justify-content: center; gap: 6px; border: 1px solid var(--apple-red); background: #fff; border-radius: 999px; padding: .42em .8em; cursor: pointer; color: var(--apple-red); font-weight: 550; font-size: 1.3rem; transition: .15s; text-decoration: none; }',
      ...
      '.lz-info td { background: #ffffff; border-radius: 0 8px 8px 0; padding: 10px 12px; border: 1px solid #f0e2de; border-left: 1px dashed #f0e2de; color: var(--ink-dark); word-break: break-word; }',
      '.lz-sns { display: flex; gap: 10px; flex-wrap: wrap; padding: 12px; }',
  ...
  function render(card) {
    ...
    MODAL.innerHTML = [
      ...
      (window.innerWidth >= 769 ? '    <button class="lz-btn lz-pdf-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><span class="lz-label">印刷</span></button>' : ''),
