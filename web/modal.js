/**
 * modal.js - 詳細表示・機能コンポーネント (完全版)
 * 役割: モーダル構築、ギャラリー制御、共有、SNS、QR/PDF自動生成(文字化け対策済)
 */
window.lzModal = (function() {
  ...
  var injectStyles = function() {
    ...
    style.textContent = `
      ...
      .lz-shell { position: relative; display: flex; align-items: center; justify-content: center; padding: env(safe-area-inset-top) 0 env(safe-area-inset-bottom); width: 100%; }
      ...
      .lz-info { margin: 12px 12px 0; width: calc(100% - 24px); border-collapse: separate; border-spacing: 0 var(--table-row-gap, 6px); }
      ...
      .lz-sns a[data-sns="web"] { background: #5b667a; }
      .lz-sns a[data-sns="ec"] { background: #0ea5e9; }
      .lz-sns a[data-sns="ig"] { background: #E4405F; }
      .lz-sns a[data-sns="fb"] { background: #1877F2; }
      .lz-sns a[data-sns="x"] { background: #000000; }
      .lz-sns a[data-sns="line"] { background: #06C755; }
      ...
      .lz-arrow svg { width: 20px; height: 20px; stroke: var(--apple-red); stroke-width: 2; fill: none; }
      .lz-prev { left: -60px; } .lz-next { right: -60px; }
      @media(max-width:768px) { .lz-prev, .lz-next { top: auto; bottom: -54px; left: 50%; } .lz-prev { transform: translateX(-120%); } .lz-next { transform: translateX(20%); } }
    `;
  ...
  function buildInfoTableHTML(d){
    ...
    if (d.fee) rows.push('<tr><th>参加費</th><td>' + C.esc(d.fee) + '</td></tr>');
    if (d.target) rows.push('<tr><th>対象</th><td>' + C.esc(d.target) + '</td></tr>');
    if (d.org) rows.push('<tr><th>主催</th><td>' + C.esc(d.org) + '</td></tr>');
    
    var links = [];
    if (d.form) links.push('<a href="' + C.esc(d.form) + '" target="_blank" rel="noopener">フォーム</a>');
  ...
  function buildSNSIconsHTML(d){
    var sns = {}; try { sns = JSON.parse(d.sns || "{}"); } catch(e){}
    var btns = [];
    var add = function(url, key, label) { if(url) btns.push('<a class="lz-sns-btn" data-sns="'+key+'" href="'+C.esc(url)+'" target="_blank" rel="noopener" aria-label="'+label+'">'+ICON[key]+'</a>'); };
    add(d.home, "web", "HP");
    add(d.ec, "ec", "EC");
    add(sns.instagram, "ig", "Instagram");
    add(sns.facebook, "fb", "Facebook");
    add(sns.x, "x", "X");
    add(sns.line, "line", "LINE");
    add(sns.tiktok, "tt", "TikTok");
    return btns.length ? '<div class="lz-sns">' + btns.join("") + '</div>' : "";
  }

  /* ==========================================
     3. PDF生成ロジック (文字化け・QR・比率対策)
     ========================================== */
  ...
  async function generatePdf(element, title, cardId) {
    ...
    try {
      ...
      var url = new URL(window.location.href); url.searchParams.set('id', cardId);
      ...
      new QRCode(qrDiv, { text: url.toString(), width: 128, height: 128, correctLevel: QRCode.CorrectLevel.L });
      ...
      if(qrCanvas) pdf.addImage(qrCanvas.toDataURL("image/png"), "PNG", pageW - margin - 20, pageH - margin - 20, 20, 20);
  ...
  function render(card) {
    var d = card.dataset;
    document.title = d.title + " | " + C.originalTitle;
    ...
    MODAL.querySelector(".lz-share").onclick = function() {
      var url = window.location.origin + window.location.pathname + "?id=" + d.id;
      var text = C.RED_APPLE + d.title + C.GREEN_APPLE + "\n" + (d.lead || "") + "\n" + url;
      if(navigator.share) navigator.share({ title: d.title, text: d.lead, url: url });
      else { var ta=document.createElement("textarea"); ta.value=text; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta); alert("紹介文をコピーしました"); }
    };
    if(MODAL.querySelector(".lz-pdf")) MODAL.querySelector(".lz-pdf").onclick = function(){ generatePdf(MODAL, d.title, d.id); };
    
    var mainImg = MODAL.querySelector("#lz-mainimg");
    if(MODAL.querySelector(".lz-g")) {
      MODAL.querySelector(".lz-g").onclick = function(e) {
        ...
        setTimeout(function(){
          ...
          var all = MODAL.querySelectorAll(".lz-g img");
