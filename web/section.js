/**
 * section.js - 記事一覧コンポーネント (Refined Harvest Edition)
 * 役割: 記事カード生成、スクロール連動ホバー(スマホ)、葉っぱのアクセント
 */
(function() {
  "use strict";
  ...
  var injectStyles = function() {
    ...
    style.textContent = [
      ...
      '}',
      /* PC用ホバーとスマホ用アクティブクラスの共通設定 */
      '.lz-card:hover, .lz-card.is-active {',
      '  transform: translateY(-8px) scale(1.01);', /* 上がり幅を抑制 */
      '  border-color: var(--apple-red);',
      '  box-shadow: 0 20px 40px -10px rgba(207, 58, 58, 0.12), 0 10px 20px -15px rgba(0, 0, 0, 0.2);',
      '}',
      ...
      /* ★タイトル左側の葉っぱアクセント */
      '.lz-title-sm {',
      '  margin: 0; font-weight: 700; font-size: 1.6rem; color: var(--ink-dark); line-height: 1.4;',
      '  position: relative; padding-left: 0; transition: padding-left 0.3s ease;',
      '}',
      '.lz-card:hover .lz-title-sm, .lz-card.is-active .lz-title-sm { padding-left: 22px; color: var(--apple-red); }',
      '.lz-title-sm::before {',
      '  content: ""; position: absolute; left: 0; top: 50%; transform: translateY(-50%) rotate(-15deg);',
      '  width: 14px; height: 14px; background: var(--apple-green); border-radius: 0 80% 0 80%;',
      '  opacity: 0; transition: opacity 0.3s ease, transform 0.3s ease;',
      '}',
      '.lz-card:hover .lz-title-sm::before, .lz-card.is-active .lz-title-sm::before { opacity: 1; transform: translateY(-50%) rotate(0deg); }',

      '.lz-lead { font-weight: 400; font-size: 1.25rem; line-height: 1.7; color: var(--ink-light); }',

      /* ロード画面 (黄金比維持) */
      '.lz-loading { position: relative; display: flex; align-items: center; justify-content: center; height: 360px; border: 1px dashed var(--border); border-radius: 12px; background: #fffaf8; }',
      ...
      '.lz-track-outer::after { content: ""; position: absolute; top: 0; right: 0; width: 60px; height: 100%; background: linear-gradient(to right, rgba(255,255,255,0), rgba(255,255,255,1)); pointer-events: none; z-index: 2; }',
      '.lz-track { display: grid; grid-auto-flow: column; grid-auto-columns: var(--cw, calc((100% - 32px) / 3.2)); gap: 20px; overflow-x: auto; padding: 12px 12px 40px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none; }',
      ...
      '.lz-media > img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s ease; }',
      '.lz-card:hover .lz-media > img, .lz-card.is-active .lz-media > img { transform: scale(1.08); }',
  ...
  function cardHTML(it, pad, groupKey) {
    ...
    return [
      '<article class="lz-card" data-id="' + C.esc(title) + '" data-id-raw="' + title + '" data-title="' + C.esc(title) + '"',
  ...
  window.renderSection = async function(root) {
    ...
    try {
      ...
      /* ★スマホ用：スクロール連動ホバー (もっとも見えているカードを浮かせる) */
      if (mql.matches) {
        var tracks = root.querySelectorAll(".lz-track");
        var observerOptions = { root: null, threshold: 0.8 };
        ...
        root.querySelectorAll(".lz-card").forEach(function(card) {
