/**
 * section.js - 記事一覧コンポーネント (センター調整・完全復元版)
 * 役割: 記事カード生成、読み込みアニメーション(数学的センター調整)、プレースホルダー、自動再生
 */
(function() {
  "use strict";
  ...
  var injectStyles = function() {
    ...
    style.textContent = [
      '.lz-section { margin: 48px 0; position: relative; min-height: 400px; }',
      ...
      /* ★ローディング中心の修正点★ */
      '.lz-loading { display: flex; align-items: center; justify-content: center; height: var(--loading-h, 320px); border: 1px dashed var(--border); border-radius: 12px; background: #fffaf8; }',
      '.lz-loading-inner { display: flex; flex-direction: column; align-items: center; gap: 8px; color: #a94a4a; width: 100%; }',
      '.lz-logo { width: 140px; height: 140px; margin: 0 auto; display: block; overflow: visible; }', /* margin-leftを廃止 */
      ...
      '.lz-loading-label { font-weight: 600; letter-spacing: .02em; font-size: 1.4rem; text-align: center; }'
    ].join('\n');
    document.head.appendChild(style);
  };

  /* ==========================================
     2. ユーティリティ: SVGセンター精密調整
     ========================================== */
  function lzCenterLogoSVG(svg){
    try {
      var path = svg.querySelector('path');
      if(!path) return;
      var bb = path.getBBox(); // パスの実測サイズを取得
      var cx = bb.x + bb.width / 2;
      var cy = bb.y + bb.height / 2;
      var box = Math.max(bb.width, bb.height);
      // パスの中心をViewBoxの真ん中に据える
      svg.setAttribute('viewBox', (cx - box/2) + ' ' + (cy - box/2) + ' ' + box + ' ' + box);
    } catch(e){
      // 失敗時のフォールバック
      svg.setAttribute('viewBox', "-60 -60 720 720");
    }
  }
  ...
  window.renderSection = async function(root) {
    ...
    root.innerHTML = [
      ...
      '        <svg class="lz-logo" viewBox="0 0 1 1" preserveAspectRatio="xMidYMid meet">', /* 初回は仮ViewBox */
