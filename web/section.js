/**
 * section.js - 記事一覧コンポーネント (style.css完全再現版)
 * 役割: 記事カード生成、中央揃えロードアニメ(サイズ大・マージン調整)、画像なしプレースホルダー
 */
(function() {
  "use strict";
  ...
  var injectStyles = function() {
    ...
    style.textContent = [
      /* --- Root Variables (一部再定義) --- */
      ':root { --loading-h: clamp(220px, 38vh, 360px); }',

      /* --- Section & Layout --- */
      '.lz-section { visibility: hidden; margin: 48px 0; position: relative; contain: content; }',
      '.lz-section.lz-ready { visibility: visible; }',
      '.lz-head { margin: 0 0 16px; padding: 0; position: relative; z-index: 10; }',
      ...
      /* ★ここが重要：大きさ160pxを維持し、位置だけを左に70pxずらす★ */
      ...
      '.lz-track { display: grid; grid-auto-flow: column; grid-auto-columns: var(--cw, calc((100% - 32px) / 3.2)); gap: 18px; overflow: auto; padding: 12px 12px 32px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none; }',
  ...
  window.renderSection = async function(root) {
    ...
    root.innerHTML = [
      ...
      '        <svg class="lz-logo" viewBox="-60 -60 720 720" preserveAspectRatio="xMidYMid meet" aria-hidden="true" style="overflow:visible">',
