/**
 * section.js - 記事一覧コンポーネント (最終完成版)
 * 役割: 記事カード生成、中央揃えロードアニメ(サイズ大)、画像なし透かし、自動再生
 */
(function() {
  "use strict";
  ...
  /* ==========================================
     1. CSS (サイズ160px維持・視覚的センター調整・枠確保)
     ========================================== */
  var injectStyles = function() {
    ...
    style.textContent = [
      /* 記事読み込み前の高さを確保し、ガタつきを防止 */
      '.lz-section { margin: 48px 0; position: relative; min-height: 520px; }', 
      ...
      /* ローディング：配置の精度を極限まで高める */
      '.lz-loading { display: flex; align-items: center; justify-content: center; height: 420px; border: 1px dashed var(--border); border-radius: 12px; background: #fffaf8; }',
      '.lz-loading-inner { display: flex; flex-direction: column; align-items: center; justify-content: center; width: 160px; }',
      
      /* ★ロゴサイズを160pxの迫力あるサイズに固定。
         座標データのズレを transform で視覚的に補正し、下の文字と中心を合わせる */
      '.lz-logo { width: 160px; height: 160px; display: block; overflow: visible; transform: translateX(-35px); }',
      ...
      /* 読み込み中テキスト：ロゴの補正値に合わせて位置を同期 */
      '.lz-loading-label { margin-top: 12px; font-weight: 600; font-size: 1.45rem; color: #a94a4a; letter-spacing: .1em; white-space: nowrap; }',
  ...
  window.renderSection = async function(root) {
    ...
    root.innerHTML = [
      ...
      '        <svg class="lz-logo" viewBox="0 0 720 720" preserveAspectRatio="xMidYMid meet">',
      ...
      '        </svg>',
      '        <div class="lz-loading-label">読み込み中...</div>',
      '      </div>',
      '    </div>',
      '  </div>',
      '</div>'
