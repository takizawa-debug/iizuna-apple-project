...
(function () {
  ...
  window.renderSection = async function (root) {
    ...
    try {
      ...
      console.error("[lzSection] error:", e);
      var errorLabel = window.LZ_CURRENT_LANG === 'ja' ? '読み込みに失敗しました' : 'Failed to load content';
      var g = root.querySelector(".lz-groupwrap");
      if (g) g.innerHTML = '<div style="padding:40px; text-align:center; color:#999;">' + C.esc(errorLabel) + '</div>';
    }
  };
