/**
 * section.js - 記事表示コンポーネント
 * 役割: 記事カードの生成、カルーセル制御、チラ見せ演出の管理
 */
(async function() {
  "use strict";
  ...
  const config = await new Promise(r => { 
    const c = () => window.LZ_CONFIG ? r(window.LZ_CONFIG) : setTimeout(c, 50); 
    c(); 
  });

  /* ==========================================
     1. CSSの注入 (記事・カード・カルーセル)
     ========================================== */
  const injectSectionStyles = () => {
    ...
    style.textContent = `
      /* セクション全体 */
      .lz-section { visibility: hidden; margin: 48px 0; position: relative; contain: content; }
      .lz-section.lz-ready { visibility: visible; }

      /* L2タイトル（赤帯） - マスクの影響を受けないようz-indexを設定 */
      ...
      .lz-track { 
        ...
        gap: 18px; overflow: auto; padding: 12px 12px 32px; 
  ...
  function cardHTML(it, pad, groupKey) {
    ...
    return `
      <article class="lz-card" id="${esc(title)}" data-id="${esc(title)}" data-title="${esc(title)}" 
        ...
        data-address="${esc(it.address||"")}" data-form="${esc(it.form||"")}" data-email="${esc(it.email||"")}" data-tel="${esc(it.tel||"")}"
        data-home="${esc(it.home||"")}" data-ec="${esc(it.ec||"")}" data-group="${esc(groupKey)}">
  ...
  function setupAutoPlay(track, { interval=4000, stepCards=1 } = {}){
    ...
    function tick(){
      let next = track.scrollLeft + (Math.max(1, stepCards) * cardW());
  ...
  window.renderSection = async function(root) {
    ...
    const sec = root.querySelector(".lz-section");
    try {
      ...
      const wrapTrack = (c, k) => `<div class="lz-track-outer"><div class="lz-track" data-group="${esc(k)}">${c}</div></div>`;
      ...
      root.querySelector(".lz-groupwrap").innerHTML = `<div style="padding:20px; text-align:center; color:#999;">記事の読み込みに失敗しました</div>`;
    }
  };
