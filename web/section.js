/**
 * section.js - 記事一覧コンポーネント (最終調整版)
 * 役割: 記事カード生成、中央揃えロードアニメ(復元)、スクロールバー、収穫UX
 */
(function() {
  "use strict";
  ...
  var injectStyles = function() {
    ...
    style.textContent = [
      ...
      '.lz-loading-label { font-weight: 550; font-size: 1.4rem; letter-spacing: .1em; padding-left: 0; }',

      /* ★スクロールバー：記事が右にあることを知らせる「収穫の道」 */
      '.lz-track-outer { position: relative; width: 100%; overflow: hidden; padding-bottom: 10px; }',
      '.lz-track {',
      '  display: grid; grid-auto-flow: column; grid-auto-columns: var(--cw, calc((100% - 32px) / 3.2)); gap: 20px;',
      '  overflow-x: auto; padding: 12px 12px 24px; scroll-snap-type: x mandatory;',
      '  -webkit-overflow-scrolling: touch;',
      '  /* モダンなスクロールバー表示 */',
      '  scrollbar-width: thin; scrollbar-color: var(--apple-red) transparent;',
      '}',
      '.lz-track::-webkit-scrollbar { height: 6px; display: block; }', /* スクロールバーを復活 */
      '.lz-track::-webkit-scrollbar-track { background: rgba(0,0,0,0.03); border-radius: 10px; }',
      '.lz-track::-webkit-scrollbar-thumb { background: var(--apple-red); border-radius: 10px; opacity: 0.6; }',
  ...
  window.renderSection = async function(root) {
    ...
    // ★重要：滝澤さんが治ったと仰った時の HTML 構造へ差し戻し
    root.innerHTML = [
    ...
    try {
      ...
      /* スマホ：スクロール連動アクティブ（ IntersectionObserver ） */
      if (mql.matches) {
